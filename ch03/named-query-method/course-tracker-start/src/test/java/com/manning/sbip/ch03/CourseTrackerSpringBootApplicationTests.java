...
class CourseTrackerSpringBootApplicationTests {
    ...
    @Test
    public void giveCoursesCreatedWhenLoadCoursesBySpringCategoryThenExpectThreeCourses(){
        courseRepository.saveAll(getCourseList());
        assertThat(courseRepository.findAllByCategoryAndRating("Spring", 4)).hasSize(1);
    }

    @Test
    public void givenCoursesCreatedWHenLoadCoursesBySpringCategoryThenExpectTwoCourses(){
        courseRepository.saveAll(getCourseList());
        Condition<Course> courseCondition = new Condition<>() {
            @Override
            public boolean matches(Course course){
                return course.getName().equals("Getting Started with Spring Security DSL") && course.getRating()==5;
            }
        };
        assertThat(courseRepository.findByRating(5)).first().has(courseCondition);
    }
